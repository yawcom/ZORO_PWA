...
function createStartScreen() {
    ...
    const background = this.add.image(width/2, height/2, 'background_Start').setDisplaySize((config.height*2752)/2100, config.height);
...
function gameOver(success) {
    ...
    playAgainButton.on('pointerdown', () => {
        ...
        score = 0;
        mistakes = 0;
        initialCardSetup = false; // Reset card setup
        gameState = 'start';
        
        // Remove all input listeners again before going to start screen
        this.input.removeAllListeners();
